...
import { buttonVariants } from "../feedback/button";

export function Dialog({ ...props }: React.ComponentProps<typeof BaseDialog.Root>) {
	return <BaseDialog.Root data-slot="dialog" {...props} />;
}

export function DialogTrigger({ children, ...props }: React.ComponentProps<typeof BaseDialog.Trigger>) {
	return (
...
export function DialogPopup({ className, children, ...props }: React.ComponentProps<typeof BaseDialog.Popup>) {
	return (
		<BaseDialog.Portal>
			<BaseDialog.Backdrop
				className={cn(
					"fixed inset-0 z-50 min-h-dvh bg-black/60 transition-[color,opacity] backdrop-blur-sm",
...
export function DialogHeader({ className, children, ...props }: React.ComponentProps<"header">) {
	return (
		<header data-slot="dialog-header" {...props} className={cn("px-6 pt-4.5 flex items-center gap-3.5", className)}>
			{children}
		</header>
	);
}

export function DialogTitle({ className, children, ...props }: React.ComponentProps<typeof BaseDialog.Title>) {
	return (
		<BaseDialog.Title data-slot="dialog-title" {...props} className={cn("text-xl font-semibold", className)}>
			{children}
		</BaseDialog.Title>
	);
}
...
export function DialogBody({ className, children, ...props }: React.ComponentProps<"div">) {
	return (
		<div data-slot="dialog-body" {...props} className={cn("px-6 py-4.5 space-y-1.5", className)}>
			{children}
		</div>
	);
}
...
export function DialogClose({ className, children, render, ...props }: React.ComponentProps<typeof BaseDialog.Close>) {
	return (
		<BaseDialog.Close
