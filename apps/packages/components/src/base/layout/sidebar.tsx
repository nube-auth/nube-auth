...
export function SidebarItem({ className, children }: React.ComponentProps<"li">) {
	return (
		<li
			...
			className={cn("group/sidebar-item", "**:data-[slot=sidebar-submenu]:w-auto relative flex", className)}
		>
			{children}
		</li>
	);
...
>(function SidebarItemButton({ className, render, active, expandableIndicator = true, ...props }, ref) {
	return useRender({
...
export function SidebarSubmenu({ className, ...props }: React.ComponentProps<"div">) {
	return (
		<div data-slot="sidebar-submenu" {...props} className={cn("py-0.5 border-l border-border w-full", className)} />
	);
}
