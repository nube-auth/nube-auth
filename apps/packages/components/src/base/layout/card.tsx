...
export function Card({ className, ...props }: React.ComponentProps<"div">) {
	return (
		<div
			data-slot="card"
			className={cn("text-foreground ring ring-card-border rounded-xl shadow-card bg-card", className)}
			{...props}
		/>
	);
}
...
export function CardHeader({
	...
	return <header data-slot="card-header" className={cn(cardHeaderVariants({ align, className }))} {...props} />;
}
...
export function CardHeaderAction({ className, ...props }: React.ComponentProps<"div">) {
	return (
		<div
			...
			className={cn("col-start-3 row-start-1 row-span-2 ml-auto flex items-center gap-2", className)}
			{...props}
		/>
	);
}
