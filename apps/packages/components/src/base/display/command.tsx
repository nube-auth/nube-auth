...
import { Dialog, DialogBody, DialogPopup, DialogTrigger } from "../layout/dialog";

export function Command({ ...props }: React.ComponentProps<typeof Dialog>) {
	return <Dialog {...props} />;
}

export function CommandTrigger({ className, ...props }: React.ComponentProps<typeof DialogTrigger>) {
	return <DialogTrigger data-slot="command-trigger" {...props} className={cn(className)} />;
}
...
export function CommandFooterItem({ className, ...props }: React.ComponentProps<"div">) {
	return <div data-slot="command-footer-item" {...props} className={cn("flex items-center gap-2.5", className)} />;
}

export function CommandFooterText({ className, ...props }: React.ComponentProps<"span">) {
	return (
		<span data-slot="command-footer-text" {...props} className={cn("text-sm text-muted font-medium", className)} />
	);
}
