...
export function Strong({ className, ...props }: React.ComponentProps<"strong">) {
	return <strong data-slot="text-strong" className={cn("font-semibold text-foreground", className)} {...props} />;
}
...
export function Code({ className, ...props }: React.ComponentProps<"code">) {
	return (
		<code
			data-slot="text-code"
			className={cn('font-mono text-sm text-foreground after:content-["`"] before:content-["`"]', className)}
			{...props}
		/>
	);
}
