...
export function MeterLabel({ className, ...props }: React.ComponentProps<typeof BaseMeter.Label>) {
	return (
		<BaseMeter.Label data-slot="meter-label" {...props} className={cn("font-medium text-foreground", className)} />
	);
}

export function MeterValue({ className, ...props }: React.ComponentProps<typeof BaseMeter.Value>) {
	return <BaseMeter.Value data-slot="meter-value" {...props} className={cn("text-sm text-dimmed", className)} />;
}
