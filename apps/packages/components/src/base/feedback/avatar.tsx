...
export const avatarVariants = cva(
	"relative rounded-full select-none bg-avatar ring-1 ring-border/70 flex items-center justify-center text-avatar-foreground font-semibold",
	{
		variants: {
			size: {
				sm: "size-8",
...
export function Avatar({
	...
	return <BaseAvatar.Root data-slot="avatar" className={cn(avatarVariants({ size, className }))} {...props} />;
}

export function AvatarImage({ className, ...props }: React.ComponentProps<typeof BaseAvatar.Image>) {
	return <BaseAvatar.Image data-slot="avatar-image" className={cn("size-full rounded-full", className)} {...props} />;
}
...
export const avatarIndicatorVariants = cva("absolute flex items-center justify-center size-2.5 rounded-full text-xs", {
	variants: {
		position: {
			top: "top-0 right-0",
