...
export const toggleGroupVariants = cva(
	["flex items-center gap-0.5 ring rounded p-0.5", "*:data-[slot=toggle]:shadow-none", "*:data-[slot=toggle]:ring-0"],
	{
		variants: {
			variant: {
...
export function ToggleGroup({
	...
	variant,
	...props
}: React.ComponentProps<typeof BaseToggleGroup> & VariantProps<typeof toggleGroupVariants>) {
	return (
		<BaseToggleGroup
			...
			className={cn(toggleGroupVariants({ variant, orientation, size, className }))}
			{...props}
		/>
	);
}
