...
export function Modal({ isOpen, onClose, children, size = "md" }: ModalProps) {
	...
	const sizeClass = size === "sm" ? "w-96" : size === "lg" ? "w-2xl" : "w-xl";

	return (
		<Dialog open={isOpen} onOpenChange={onClose}>
			<DialogPopup className={sizeClass}>{children}</DialogPopup>
		</Dialog>
	);
}
...
export function ModalHeader({ children, onClose }: ModalHeaderProps) {
	return (
		<DialogHeader className="flex items-center justify-between pr-8">
			...
			{onClose && (
				<Button variant="plain" size="sm" onClick={onClose} className="absolute right-4 top-4">
