...
export function AppSetupPage() {
	...
	const [formData, setFormData] = useState({
		...
		allowedHosts: [""],
		sessionTtlDays: 28,
		enabledProviders: ["google", "github"] as string[],		requiresLicensing: false,
		defaultLicensePlan: {
			name: "Free Plan",
			slug: "",
	...
	const handleSubmit = (e: React.FormEvent) => {
		...
		if (formData.requiresLicensing) {
			dataToSend.defaultLicensePlan = {
				...
				slug: formData.defaultLicensePlan.slug || formData.defaultLicensePlan.name.toLowerCase().replace(/\s+/g, "-"),
			};
		}
	...
	return (
		<div className="page" style={{ background: "var(--content-bg)", minHeight: "100vh" }}>
			{/* Breadcrumb */}
			<nav style={{ display: "flex", alignItems: "center", gap: "8px", fontSize: "13px", marginBottom: "24px", padding: "20px 40px 0" }}>
			...
			{/* 2 Column Layout */}
			<div style={{ display: "grid", gridTemplateColumns: "1.4fr 1fr", gap: "40px", padding: "20px 40px 40px", maxWidth: "1600px", margin: "0 auto" }}>
				{/* Left Column - Form */}
				<div>
										...
										return (
											<button
												...
												style={{
													...
													background: isEnabled ? "var(--primary-light)" : "var(--surface-secondary)",
													cursor: "pointer",
													transition: "all 0.2s ease",
													textAlign: "left",
													position: "relative",
											...
											>
												<div style={{ display: "flex", alignItems: "center", gap: "12px" }}>
													<div style={{ display: "flex", alignItems: "center", justifyContent: "center", width: "32px", height: "32px" }}>{provider.icon}</div>
													<div style={{ flex: 1 }}>
							...
							<div style={{
								...
								<div style={{ display: "flex", flexDirection: "column", gap: "8px", fontSize: "13px" }}>
									<div style={{ display: "flex", justifyContent: "space-between" }}>
										...
										<span style={{ color: "var(--text-primary)", fontWeight: "500" }}>{formData.name}</span>
									</div>
									<div style={{ display: "flex", justifyContent: "space-between" }}>
										<span style={{ color: "var(--text-tertiary)" }}>OAuth Providers:</span>
										<span style={{ color: "var(--text-primary)", fontWeight: "500" }}>
											{formData.enabledProviders.length > 0
												? formData.enabledProviders.map(id => 
													AVAILABLE_PROVIDERS.find(p => p.id === id)?.name
												).join(", ")
												: "None"
											}
										</span>
					...
					{step === 4 && (
						<div className="space-y-6">
							<div>
								<label className="form-label" style={{ display: "flex", alignItems: "center", gap: "12px", cursor: "pointer", marginBottom: "12px" }}>
									<input
										type="checkbox"
										checked={formData.requiresLicensing}
										onChange={(e) => setFormData({ ...formData, requiresLicensing: e.target.checked })}
										style={{ width: "18px", height: "18px", cursor: "pointer" }}
									/>
									<span style={{ fontWeight: "600", fontSize: "15px" }}>Enable Licensing & Payments</span>
								</label>
								<p style={{ fontSize: "13px", color: "var(--text-tertiary)", marginLeft: "30px" }}>
									Add subscription plans and payment processing. Disable if your app only needs user management.
								</p>
							</div>
									...
									<div className="space-y-4">
										<div>
											...
											<input
												...
												onChange={(e) => setFormData({
													...formData,
													defaultLicensePlan: { ...formData.defaultLicensePlan, name: e.target.value }
												})}
												className="form-control"
										...
										<div>
											...
											<textarea
												...
												onChange={(e) => setFormData({
													...formData,
													defaultLicensePlan: { ...formData.defaultLicensePlan, description: e.target.value }
												})}
												className="form-control"
												rows={3}
										...
										<div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "16px" }}>
											<div>
												...
												<input
													...
													onChange={(e) => setFormData({
														...formData,
														defaultLicensePlan: { ...formData.defaultLicensePlan, price: parseFloat(e.target.value) || 0 }
													})}
													className="form-control"
													required={formData.requiresLicensing}
												/>
											...
											<div>
												...
												<input
													...
													onChange={(e) => setFormData({
														...formData,
														defaultLicensePlan: { ...formData.defaultLicensePlan, currency: e.target.value.toUpperCase() }
													})}
													className="form-control"
													placeholder="USD"
													maxLength={3}
										...
										<div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "16px" }}>
											<div>
												<label htmlFor="billingPeriod" className="form-label">
													Billing Period <span style={{ color: "var(--danger)" }}>*</span>
												</label>
												<select
													id="billingPeriod"
													...
													onChange={(e) => setFormData({
														...formData,
														defaultLicensePlan: { ...formData.defaultLicensePlan, billing_period: e.target.value as any }
													})}
													className="form-control"
													required={formData.requiresLicensing}
												>
											...
											<div>
												...
												<input
													...
													onChange={(e) => setFormData({
														...formData,
														defaultLicensePlan: { ...formData.defaultLicensePlan, trial_days: parseInt(e.target.value) || 0 }
													})}
													className="form-control"
												/>
											</div>
					...
					<div style={{ position: "sticky", top: "20px" }}>
						<div className="card" style={{ padding: "24px", background: "var(--surface-secondary)" }}>
							...
							<div style={{
								...
								<svg style={{ width: "16px", height: "16px", color: "var(--primary)", flexShrink: 0, marginTop: "2px" }} fill="none" stroke="currentColor" viewBox="0 0 24 24">
									<path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M13 16h-1v-4h-1m1-4h.01M21 12a9 9 0 11-18 0 9 9 0 0118 0z" />
								</svg>
								<p style={{ fontSize: "12px", color: "var(--text-secondary)", margin: 0, lineHeight: "1.5" }}>
									This preview shows the login screen with your app's branding and selected OAuth providers
								</p>
							</div>
						</div>
					</div>
