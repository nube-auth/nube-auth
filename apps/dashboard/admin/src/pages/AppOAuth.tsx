	...
	const handleToggleProvider = async (provider: OAuthProvider) => {
		...
		try {
			if (isSelected) {
				...
				showToast(`${provider.provider} disabled`, "success");
			} else {
				await selectProviderMutation.mutateAsync({
					oauthProviderId: provider.id,
					displayOrder: selectedProviders.length,
				});
				showToast(`${provider.provider} enabled`, "success");
			}
		} catch (error) {
			showToast(`Failed to ${isSelected ? "disable" : "enable"} provider`, "error");
		}
	};
		...
		<div className="page">
			...
			<div style={{ marginBottom: "32px" }}>
				...
				<p style={{ fontSize: "14px", color: "var(--text-tertiary)" }}>
					Select which OAuth providers to enable for {app.name}. All providers are configured by Proofa.
				</p>
			</div>
			...
			<div style={{
				...
				<svg style={{ width: "20px", height: "20px", color: "var(--primary)", flexShrink: 0 }} fill="none" stroke="currentColor" viewBox="0 0 24 24">
				...
				<div>
					...
					<p style={{ fontSize: "13px", color: "var(--text-secondary)", margin: 0 }}>
						All OAuth providers are pre-configured by Proofa. Simply toggle them on to enable social login for your app.
					</p>
				</div>
			</div>
			...
			{!availableProviders || availableProviders.length === 0 ? (
				<div className="card" style={{ padding: "64px 24px", textAlign: "center" }}>
					...
					<p style={{ fontSize: "14px", color: "var(--text-tertiary)" }}>
						Contact support to enable OAuth providers for your account.
					</p>
				</div>
			) : (
				<div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fill, minmax(320px, 1fr))", gap: "20px" }}>
					{availableProviders.map((provider) => {
						const isSelected = selectedProviders.some((p: SelectedProvider) => p.id === provider.id);
						const isLoading = selectProviderMutation.isPending || deselectProviderMutation.isPending;
						...
						return (
							<div
								...
								onClick={() => !isLoading && handleToggleProvider(provider)}
							>
								<div style={{ display: "flex", alignItems: "flex-start", gap: "16px", marginBottom: "16px" }}>
									...
									<div style={{ flex: 1 }}>
										<div style={{ display: "flex", alignItems: "center", gap: "8px", marginBottom: "4px" }}>
											<h3 style={{ fontSize: "16px", fontWeight: "600", color: "var(--text-primary)", margin: 0, textTransform: "capitalize" }}>
												{provider.provider}
											</h3>
											<span style={{
												padding: "2px 8px",
								...
								<div style={{ 
									...
									<span style={{ fontSize: "14px", fontWeight: "500", color: isSelected ? "var(--primary)" : "var(--text-secondary)" }}>
