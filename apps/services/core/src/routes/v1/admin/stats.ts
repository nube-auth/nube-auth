...
statsRouter.get("/stats", async (c: Context) => {
	try {
		...
		for (const project of allProjects) {
			...
			const revenueByCurrency = await paymentTransactionQueries.getRevenueByProjectIdGroupedByCurrency(
				db,
				project.id,
			);
...
statsRouter.get("/:projectId/stats", async (c: Context) => {
	try {
		...
		const licenseEntries = await Promise.all(
			apps.map(
				async (a) =>
					[a.id, (await licenseQueries.findByAppId(db, a.id)).filter((l) => !l.is_test)] as [
						number,
						Awaited<ReturnType<typeof licenseQueries.findByAppId>>,
					],
			),
		);
		...
		const planIds = [...new Set([...licensesByApp.values()].flat().map((l) => l.plan_id))];
