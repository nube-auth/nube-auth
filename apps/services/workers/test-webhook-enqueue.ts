...
async function test() {
	...
	console.log("\nTo monitor:");
	console.log('  psql "postgresql://nube-auth:nube-auth_dev_password@localhost:5432/nube-auth" -c "SELECT id, status, processing_started_at FROM webhook_logs WHERE id = 7"');
