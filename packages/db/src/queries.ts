...
export const emailVerificationQueries = {
	async findByEmail(db: DbClient, email: string) {
		return db.select().from(email_verifications).where(eq(email_verifications.email, email)).get();
	},
	...
	async update(db: DbClient, id: number, data: Partial<typeof email_verifications.$inferInsert>) {
		return db.update(email_verifications).set(data).where(eq(email_verifications.id, id)).returning().get();
	},

	async delete(db: DbClient, id: number) {
		return db.delete(email_verifications).where(eq(email_verifications.id, id)).returning().get();
	},
	...
	async incrementAttempts(db: DbClient, id: number) {
		...
		return db.update(email_verifications).set(updateData).where(eq(email_verifications.id, id)).returning().get();
	},
